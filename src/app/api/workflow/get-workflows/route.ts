...
import { GetWorkflowsQuery, WorkflowResponse } from "@/interface/workflow.interface";
...
const getWorkflowsHandler = async (
  ...
  try {
    ...
    // Sort configuration
    const sortField = ['name', 'createdAt', 'updatedAt'].includes(query.sortBy as string) 
      ? query.sortBy 
      : 'createdAt';
    ...
    // Format response
    const workflowsResponse: WorkflowResponse[] = workflows.map(workflow => ({
      id: workflow.id,
      name: workflow.name,
