...
const workflowSchema = z.object({
  ...
  // These will be populated with empty arrays initially
  nodes: z.any().default([]),
  edges: z.any().default([]),
});
...
export default function WorkflowCreateButton() {
  const apiService = new ApiService("/workflow/create");
  ...
  const [cronUnit, setCronUnit] = useState<"minutes" | "hours" | "days" | "weeks" | "months">("hours");
  ...
  const addUser = (user: User) => {
    if (!selectedUsers.some((u) => u.id === user.id)) {
      ...
      // Update the form value with just the user IDs
      setValue("sharedWith", newUsers.map((u) => u.id));
    }
    setUserSearchQuery("");
    setShowUserResults(false);
  };
  ...
  const getCronExpression = () => {
    ...
    switch (cronUnit) {
      ...
      case "weeks":
        return `0 0 * * ${cronInterval === 1 ? "0" : `*/${cronInterval * 7}`}`;
      case "months":
  ...
  const onSubmit = async (data: WorkflowFormData) => {
    try {
      // Generate cron expression if scheduled
      ...
      const workflowData:CreateWorkflowRequest = {
        ...
        // Initial empty nodes and edges for a new workflow
        nodes: [],
        edges: [],
      };
      ...
      const response = await apiService.create<any, CreateWorkflowRequest>(workflowData);
      ...
      <DialogContent className="px-1 py-8">
        <ScrollArea className="sm:max-w-[600px] max-h-[80vh] px-5">
          <DialogHeader>
            ...
            <DialogDescription>
              Define your workflow details. You will be able to
              design the workflow nodes after creation.
            </DialogDescription>
          </DialogHeader>
          ...
          >
            ...
            <div className="space-y-2">
              ...
              <Select
                onValueChange={(value: "MANUAL" | "SCHEDULED" | "WEBHOOK") => 
            ...
            {showScheduler && (
              <div className="space-y-2">
                ...
                <div className="text-sm text-gray-500 mt-1">
                  <p>
                    Will run every {cronInterval || 1} {cronUnit || "hours"}
                  </p>
